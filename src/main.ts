...
import fs from 'fs';
import { NewFile, Compiled, Print } from './api';
...
async function ReadAll(file: Readable) {
    const chunks: Buffer[] = [];
    for await (let chunk of file) chunks.push(chunk)
...
async function main() {
    program
        ...
        .action(async function (src, dst) {
            let source: string;
            if (src === STDIN) source = await ReadAll(Readable.from(process.stdin));
            else source = await ReadAll(Readable.from(fs.createReadStream(src)));
            const input: Combined = JSON.parse(source);

            const compiled: Compiled[] = [];
            for (let k in input.contracts) {
                if (!input.contracts[k].abi) throw new Error(`ABI not given for: ${k}`);
                if (!input.contracts[k].bin) throw new Error(`Bin not given for: ${k}`);
                compiled.push({ 
                    name: k, 
