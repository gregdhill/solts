...
import { CollapseInputs, CombineTypes, Signature, ContractMethodsList } from './solidity';
...
function output(signatures: Array<Signature>, client: ts.Identifier, provider: Provider): ts.Block {
    ...
    return ts.createBlock([
        ...signatures.filter(sig => sig.inputs.length > 0).map(sig => {
            return ts.createIf(join(
                ...sig.inputs.map(input => {
                    return ts.createStrictEquality(ts.createTypeOf(ts.createIdentifier(input.name)), ts.createLiteral('string'))
                })
        ...
        ts.createArrowFunction(undefined, [provider.getTypeArgumentDecl()], [CreateParameter(client, provider.getTypeNode())], undefined, undefined, ts.createBlock([
            ts.createReturn(ts.createObjectLiteral(methods.filter(method => method.type === 'function').map(method => {
                    ...
                    return ts.createPropertyAssignment(method.name, ts.createArrowFunction(
                        undefined,
                        undefined,
                        Array.from(CollapseInputs(method.signatures), ([key, value]) => CreateParameter(key, CombineTypes(value))),
                        undefined,
                        undefined,
