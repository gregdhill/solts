...
function output(decodeFn: ts.CallExpression, sig: Signature): ts.Block {
    let named = [];
    if (sig.outputs && sig.outputs[0] !== undefined) named = sig.outputs.filter(out => out.name !== "")
    ...
    if (sig.outputs.length !== 0 && sig.outputs.length === named.length) {
        const setter = ts.createVariableStatement([], ts.createVariableDeclarationList(
            [ts.createVariableDeclaration(ts.createArrayBindingPattern(sig.outputs.map(out => ts.createBindingElement(undefined, undefined, out.name))), undefined, decodeFn)], ts.NodeFlags.Const));
    
        return ts.createBlock([
            setter,
            ts.createReturn(ts.createObjectLiteral(sig.outputs.map(out => ts.createPropertyAssignment(out.name, ts.createIdentifier(out.name)))))
        ], true);
    } else {
        return ts.createBlock([
            ts.createReturn(sig.outputs.length > 0 ? decodeFn : undefined)
        ])
    }
}
...
function decoder(sig: Signature, client: ts.Identifier, provider: Provider, data: ts.Identifier) {
    let args = [];
    if (sig.outputs && sig.outputs[0] !== undefined) args = sig.outputs.map(arg => ts.createLiteral(arg.type));
...
export const Decode = (methods: ContractMethodsList, provider: Provider) => {
    ...
    return DeclareConstant(DecodeName, 
        ts.createArrowFunction(undefined, [provider.getTypeArgumentDecl()], [
            ...
            ts.createReturn(ts.createObjectLiteral(methods.filter(method => method.type === 'function').map(method => {
                return ts.createPropertyAssignment(method.name, ts.createArrowFunction(
                    undefined,
                    undefined,
                    [],
